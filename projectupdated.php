...
require('connect.php');
	...
	if (isset($_POST['project'])){
		...
		$query = "UPDATE projectdetails SET jobname = '$project'
										  WHERE jobid = '$jobid'";
	...
	if (isset($_POST['sinkbit'])){
		...
		if ($sinkdate == NULL){
			$query = "UPDATE projectstatus SET sinkdate = CURRENT_TIMESTAMP
										  WHERE jobid = '$jobid'";
			if (!mysqli_query($connection, $query)) {
				die('Error: ' . mysqli_error($connection));
			}
		...
		$sinkbit = 0;
		$query = "UPDATE projectstatus SET sinkdate = NULL
										  WHERE jobid = '$jobid'";
		if (!mysqli_query($connection, $query)) {
			die('Error: ' . mysqli_error($connection));
		}
	...
	if (isset($_POST['billedbit'])){
		...
		if ($billeddate == NULL){
			$query = "UPDATE projectstatus SET billeddate = CURRENT_TIMESTAMP
										  WHERE jobid = '$jobid'";
			if (!mysqli_query($connection, $query)) {
				die('Error: ' . mysqli_error($connection));
			}
		...
		$billedbit = 0;
		$query = "UPDATE projectstatus SET billeddate = NULL
										  WHERE jobid = '$jobid'";
		if (!mysqli_query($connection, $query)) {
			die('Error: ' . mysqli_error($connection));
		}
